...
| `patentscope` | 无字段算子时按查询式是否含非 ASCII 字符选字段（含中文/日文/韩文等包成 `FP:(…)`，纯英文数字包成 `EN_ALLTXT:(…)`）；抽出 `country=XX`/`ctr=XX` 拼进 `AND CTR:(XX)`；词数 >2 时再追加"最长两词""最长一词"两个收窄候选，逐个试、命中即停 | `FP:(配电变压器 故障诊断)` → 10 命中且全部主题相关；`FP:(物化视图 增量刷新)` → 10 命中且全部主题相关。**2026-09-26 更正**：早期版本对中文一律包成 `EN_ALLTXT:(…)`（英文机翻全文字段），命中数看似正常但内容常与查询词无关（如 `EN_ALLTXT:(大模型 评测 缓存)` 命中全是无关内容），已改用 `FP` 修复，不要再参考旧版判断 |
