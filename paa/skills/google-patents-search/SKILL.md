...
- **曾经不可信，已定位并修复**：`patentscope` 后端的中文关键词全文检索。根因：`EN_ALLTXT` 是
  **英文机器翻译全文字段**，塞中文查询词进去命中与否、相关与否都不稳定（同一天测出"配电变压器
  故障诊断"能凑巧命中电网相关结果，但"大模型 评测 缓存""题目版本 依赖失效 重算"命中的全是无关
  内容）；换成 `FP`（Front Page：标题/摘要/申请人等前页字段，覆盖各文献原始语言）后，同样的中文
  查询词精准命中相关文献（如 `FP:(物化视图 增量刷新)` 命中多篇物化视图增量刷新专利）。已在
  `patentscope_query_variants()` 里按查询式是否含非 ASCII 字符自动选字段（含中文/日文/韩文等用
  `FP`，纯英文数字仍用 `EN_ALLTXT`），英文路径回归测试通过，未受影响。
