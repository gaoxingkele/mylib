...
| **google-patents-search** | **免费**专利检索与原文核验（**纯 HTTP/API，不使用浏览器**；后端=Google 站点接口 / WIPO PATENTSCOPE / Google BigQuery 官方数据集，无需 key） | incoPat 不可用/需第二轮复检索时 feeds `evidence/prior_art_search/`；`gp_verify.py` 是引用落稿的门禁 |
