...
把设计**跑出来并审计**，而不是只做描述。以下调用顺序就是本技能的执行映射；
《管理世界》重中国情境实证 + 政策可操作，识别 + 经济量级，定性/案例另循其标准。
...
正文报告**经济量级**，完整 battery 进附录；每个数字都能复现。若 StatsPAI/Stata
未连接，改用项目内 `resources/code/`，并标注未验证数字。
## 必查清单
