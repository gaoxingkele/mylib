...
按实证论文的章节顺序组织，每组标注命中的 `patterns-zh.md` 规则号。

---

## 案例 1 · 摘要段（命中 ZH01, ZH12, ZH03）
...
**修复点**：
- 删除"毋庸置疑"（ZH01 四字套话）
- "深远影响"→ 具体数字 6.8%（ZH03 AI 形容词）
- "充分证明了"→ "发现"+具体证据（ZH12 绝对化）
- 加入具体文献回应（ZH13 观点-证据分离）

---

## 案例 2 · 引言段首句（命中 ZH04, ZH07）
...
**修复点**：
- 删除段首"此外"（ZH04 显性连接词）
- "相关研究表明"→ 具体作者+年份+期刊（ZH07 主语模糊）
- 保留正反两个文献视角，体现真实学术张力

---

## 案例 3 · 文献综述段（命中 ZH11, ZH13）
...
**修复点**：
- 去掉"首先/其次/再次/最后"（ZH11 序数词）
- 每条脉络加具体数字和作者（ZH13 证据锚点）
- 承认分歧而非粉饰共识，体现学者的审慎

---

## 案例 4 · 假设提出段（命中 ZH06, ZH14）
...
**修复点**：
- 加入短句"这两条路径未必同时激活"（ZH06 句长方差 / ZH14 学术谦逊）
- "能够...显著促进" → "可能有两条路径...未必同时激活"（ZH12 绝对化 → ZH14 hedge）
- 引入"先提出基准假设"的表达，体现研究推进的真实节奏

---

## 案例 5 · 数据描述段（命中 ZH05, ZH08）
...
**修复点**：
- "主要包括" 后过多并列变成单独说明每个数据源（ZH05）
- 被动式"经过...处理"→ 主动说明具体步骤（ZH08）
- 加入数据窗口的政策背景锚点（ZH13）
...
**修复点**：
- 去掉"对于...而言"、"由于...故而"、"在此基础上"（ZH02 虚词堆叠）
- 改为直接、简洁的学术陈述
- 末句预告稳健性部分，体现真实论文的推进逻辑

---

## 案例 7 · 基准结果段（命中 ZH15, ZH12）
...
**修复点**：
- "非常显著" → 具体 t 值和系数（ZH15 主观强调）
- "充分证明" → 描述数字和区间（ZH12 绝对化）
...
**修复点**：
- 消除"首先/其次/因此...因此"的机械式总分总（ZH10）
- 以"如何解释基准结果"发问开始，制造阅读张力
- 给出具体统计值，替代重复性陈述

---

## 案例 9 · 异质性段（命中 ZH06 句长方差）
...
**修复点**：
- 短句"效应在谁身上最强？"+ 长句"这与...规模经济一致"形成节奏跳跃（ZH06）
- 短句"后者常被忽略"制造强调
- 不只报告方向，还解释背后机制

---

## 案例 10 · 稳健性段（命中 ZH04, ZH01）
...
**修复点**：
- 删除"此外...综上所述"（ZH04, ZH01）
- 每个检验对应一个具体担忧，给出具体结果
- 用"四个担忧"替代空洞的"一系列"

---

## 案例 11 · 讨论段（命中 ZH14 hedge 缺失）
...
**修复点**：
- 删除"重要的理论意义和实践价值"（ZH03）
- 加入 hedge："需要谨慎解读"/"可能递减"/"可能并非最优"（ZH14）
- 主动承认局限："无法识别长期效应"、"仅 8 年，远远不够"——这是人类学者独有的写作特征

---

## 案例 12 · 结论段（命中 ZH17 回音壁）
