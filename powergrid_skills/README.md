...
从 10 个开源 GitHub skills 仓库中精选合并的高价值组件集合，面向电网/电力系统方向论文（C2GES、MA-SQLGrid 等）的写作、核查与投稿流程。当前以本目录各组件的 `MANIFEST.md` 作为上游来源与原路径的唯一记录，不再依赖已弃用的外部克隆目录。

## 合并原则

1. **只补缺口**：审稿模拟与期刊画像环节本地已有更强系统 `D:/aicoding/paper_reviews/`（9 期刊画像 + 多智能体审稿），本包**不重复合并**相关组件；只合并其不覆盖的能力。
2. **精选拷贝**：本包保存精选文件；组件升级时按各组件 `MANIFEST.md` 记录的仓库与原路径重新对照上游。
...
- 各组件的上游仓库与原路径以相应 `MANIFEST.md` 为准；本包是**精选拷贝**，不随上游自动更新。
